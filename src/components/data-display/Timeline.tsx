      ...
      {events.map((event, index) => (
        <div key={index} className="relative grid grid-cols-[auto_1fr] gap-4">
          <div className="flex flex-col items-center">
            <div className="h-3 w-3 shrink-0 rounded-full border-2 border-border bg-surface" />
            {index < events.length - 1 && (
              <div className="mt-1 w-0.5 flex-1 bg-border-muted" />
            )}
          </div>
          <div className="pb-8 last:pb-0">
            <span className="text-caption tracking-widest text-text-muted">{event.year}</span>
