...
export function CalendarWrapper({
  ...
  const days = useMemo<CalendarDay[]>(() => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const isDayDisabled = !availableDays.includes(d.getDay()) || d < new Date(minDateStr) || d > maxDate || disabledDates.includes(dateStr);
      result.push({ date: dateStr, day: i, month, year, isCurrentMonth: true, isToday: d.toDateString() === new Date(todayStr).toDateString(), isSelected: selectedDate === dateStr, isDisabled: isDayDisabled, slots: [] });
    }
  ...
  const canGoPrev = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), 1) > new Date(minDateStr);
